...
const ThemeSwitch = ({ isDark, onToggle, className = '' }) => (
  <button
    ...
    className={`theme-switch inline-block h-7 w-[60px] shrink-0 rounded-full border shadow-sm transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-slate-200/70 sm:h-8 sm:w-[68px] ${className}`}
  >
    <span className="sr-only">{isDark ? 'Dark theme enabled' : 'Light theme enabled'}</span>
    <span className={`theme-switch-knob absolute left-1 top-1 h-5 w-5 rounded-full transition-transform duration-300 ease-out sm:h-6 sm:w-6 ${isDark ? 'translate-x-[32px] sm:translate-x-[36px]' : 'translate-x-0'}`} />
    <span className={`theme-switch-slot absolute left-1 top-1 z-10 grid h-5 w-5 place-items-center rounded-full transition-colors sm:h-6 sm:w-6 ${isDark ? 'text-amber-300' : 'theme-switch-active-text'}`}>
      <Sun size={14} fill="currentColor" strokeWidth={2.4} />
    </span>
    <span className={`theme-switch-slot absolute right-1 top-1 z-10 grid h-5 w-5 place-items-center rounded-full transition-colors sm:h-6 sm:w-6 ${isDark ? 'theme-switch-active-text' : 'text-sky-500'}`}>
